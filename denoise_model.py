...
class DenoisingUNet(nn.Module) : 
    ...
    def forward(self, x, t, cond): 
        ...
        predicted_noise = output.sample.squeeze(1)
        out = self.linear(predicted_noise)
        return out
...
def q_sample(x_start, t, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, noise=None): # simulates q(x_t | x_0) 
    if noise is None:
...
def apply_noise(x_start, t, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, noise=None): 
    if noise is None:
        noise = torch.randn_like(x_start)
...
def p_losses(denoise_model, x_start, t, cond, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, noise=None, loss_type="l1"): #modif loss_type?
    if noise is None:
        noise = torch.randn_like(x_start)
